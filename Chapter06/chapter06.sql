...
--all values from left tbale will be displayed
--only displays right tbale rows that match left table rows
--otherwise row displays as empty (null value)
--
...
--all values from right tbale will be displayed
--only displays left tbale rows that match right table rows
--otherwise row displays as empty (null value)
--
...
--displays all rows including matching rows
--
...
FROM
    ...
    AND c2010.p0010001 <> c2000.p0010001 -- only displays the rows where the population has changed
ORDER BY
    pct_change DESC;